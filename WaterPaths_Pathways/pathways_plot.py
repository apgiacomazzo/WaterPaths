...
def plot_colormap_pathways(pathways_utility, nweeks, source_colormap_id,
    ...
    ax.set_yticklabels(['Infraestrutura\nmais relevante',
                        'Infraestrutura tardia\ne menos relevante'],
...
if __name__ == "__main__":
    pathways = []
    solutions = [44]
    ### A LINHA ABAIXO GERA O ARRAY ['source 1', 'source 2', ..., 'source 30'].
    # SUBSTITUA A LINHA ABAIXO POR UM ARRAY COM OS NOMES DAS INFRAESTRUTURAS. ###
    sources = np.array(['Sistema Corumbá - Etapa 1',
                        'Sistema Corumbá - Etapa 2',
                        'Sistema Paranoá - Etapa 1',
                        'Sistema Paranoá - Etapa 2',
                        'Sistema Paranoá - Etapa 3',
                        'Descoberto - Expansão da Barragem'])
    utils_ids = [0, 1]
    files_root_directory = r'C:\Users\andre\PycharmProjects\WaterPaths_Pathways\\'
    for s in solutions:
        pathways_all_utilities = np.loadtxt(
            files_root_directory + 'Pathways_s{}_ajustado.csv'.format(s), # ARQUIVO DE PATHWAYS
            delimiter=',',
            comments='R',
            dtype=int)
        for u in utils_ids:
    ...
    ### A MATRIX ABAIXO ATRIBUI UMA COR DO COLORMAP TAB20C
    # (https://matplotlib.org/3.1.0/tutorials/colors/colormaps.html) A CADA PROJETO DE INFRAESTRUTURA--
    # O PADRÃO É [[SOURCE, COR], [SOURCE, COR], ...]. ###
    ### PARA MUDAR AS CORES DA FIGURA É SÓ MUDAR O cmap_name NA ANTEPENÚLTIMA LINHA E A MATRIX ABAIXO ###
    source_colormap_id = np.array([[6, 4],
                                   [7, 5],
                                   [9, 12],
                                   [10, 13],
                                   [11, 14],
    ...
    cb_pos = [0.2, 0.15, 0.7, 0.01] # posição da barra de cores
    ...
    for u in utils_ids:
        ...
        plot_colormap_pathways(utility_pathways_high_copy[u], 2500, source_colormap_id, solutions[0],
                               solutions[0], 5, ax[0, u], ax[1, 1], cb_pos, sources=sources,
                               cmap_name='tab20c')

    # COMENTE A LINHA ABAIXO E DESCOMENTE A SEGUINTE PARA GERAR UM SVG QUE PODE SER EDITADO NO INKSCAPE.
    #plt.show()
    plt.savefig('pathways.svg', bbox_inches='tight')